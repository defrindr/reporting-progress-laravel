    ...
    <section class="grid gap-6 {{ $isManager ? '' : 'lg:grid-cols-2' }}">
        @if (!$isManager)
            <article class="glass rounded-2xl p-5">
                ...
                @if (($isInternReadOnly ?? false) === true)
                    <div class="glass mt-4 rounded-xl border border-amber-300/50 p-3 text-sm text-amber-900">
                ...
                @elseif (($isWeekendLock ?? false) === true)
                    <div class="glass mt-4 rounded-xl border border-sky-300/50 p-3 text-sm text-sky-900">
                ...
                <form method="POST" action="{{ route('logbook.store') }}" class="mt-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input id="report_date" name="report_date" type="date" value="{{ old('report_date') }}" required @disabled(($isInternReadOnly ?? false) === true) class="w-full rounded-xl border border-white/30 bg-white/60 px-3 py-2.5 text-sm disabled:cursor-not-allowed disabled:bg-white/30 backdrop-blur-sm">
                    </div>

                    <div class="glass-dark rounded-xl border border-white/[0.06] p-3">
                        ...
                        <div class="mt-3 grid gap-2 lg:grid-cols-[1fr_1fr_auto]">
                            <select id="resume_scope" class="rounded-xl border border-white/30 bg-white/60 px-3 py-2 text-sm backdrop-blur-sm">
                                <option value="daily">Harian (tanggal report)</option>
                                <option value="weekly">Mingguan (Senin-Minggu)</option>
                            </select>

                            <label class="flex items-center gap-2 rounded-xl border border-white/30 bg-white/60 px-3 py-2 text-sm backdrop-blur-sm">
                                <input id="resume_use_ai" type="checkbox" class="rounded border-white/30" checked>
                                AI lokal tanpa API key
                            </label>

                            <button id="generate_resume_button" type="button" class="rounded-xl border border-white/30 bg-white/60 px-3 py-2 text-sm hover:bg-white/80 backdrop-blur-sm">Tarik Resume</button>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea id="done_tasks" name="done_tasks" rows="4" required @disabled(($isInternReadOnly ?? false) === true) class="w-full rounded-xl border border-white/30 bg-white/60 px-3 py-2.5 text-sm disabled:cursor-not-allowed disabled:bg-white/30 backdrop-blur-sm">{{ old('done_tasks') }}</textarea>
                    </div>

                    <div>
                        <label for="next_tasks" class="mb-2 block text-sm font-medium text-slate-700">Akan Dikerjakan</label>
                        <textarea id="next_tasks" name="next_tasks" rows="4" required @disabled(($isInternReadOnly ?? false) === true) class="w-full rounded-xl border border-white/30 bg-white/60 px-3 py-2.5 text-sm disabled:cursor-not-allowed disabled:bg-white/30 backdrop-blur-sm">{{ old('next_tasks') }}</textarea>
                    </div>

                    <div>
                        <label for="appendix_link" class="mb-2 block text-sm font-medium text-slate-700">Appendix Link (Opsional)</label>
                        <input id="appendix_link" name="appendix_link" type="url" value="{{ old('appendix_link') }}" placeholder="https://drive.google.com/..." @disabled(($isInternReadOnly ?? false) === true) class="w-full rounded-xl border border-white/30 bg-white/60 px-3 py-2.5 text-sm disabled:cursor-not-allowed disabled:bg-white/30 backdrop-blur-sm">
                    </div>

                    <button type="submit" @disabled(($isInternReadOnly ?? false) === true || ($isWeekendLock ?? false) === true) class="inline-flex w-full items-center justify-center rounded-xl bg-[#1D546D] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#163F52] disabled:cursor-not-allowed disabled:bg-slate-400">Submit Report</button>
                </form>
            </article>
        @endif
        ...
        <article class="glass rounded-2xl p-5">
            ...
            <div class="mt-4 space-y-3">
                @forelse ($logbooks as $logbook)
                    <div class="glass-dark rounded-xl border border-white/[0.06] p-3">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="rounded-full bg-white/60 px-2.5 py-1 text-xs font-medium text-slate-700 backdrop-blur-sm">{{ $logbook->status }}</span>
                        </div>
                        @if ($isManager)
                ...
                @empty
                    <p class="rounded-xl border border-dashed border-white/30 px-3 py-6 text-center text-sm text-slate-500">Belum ada logbook.</p>
                @endforelse
            </div>
        </article>
    </section>
            ...
            generateButton.addEventListener('click', async () => {
                ...
                try {
                    ...
                    previewEl.innerHTML = tasks.slice(0, 8).map((task) => {
                        ...
                        return `<li class="glass-dark rounded border border-white/[0.06] bg-white/50 px-2 py-1.5 backdrop-blur-sm">${title} | ${project} | status ${status} | due ${due}</li>`;
                    }).join('');
                } catch (error) {
